...
def test_index_scan_i_bitmap_index_scan_daju_JEDNAK_potpis():
    ...
    indeks je upotrijebio, što je cijeli cilj koncepta.
    """
    a = plan_signature(["Index Scan", "Limit"])
    b = plan_signature(["Bitmap Index Scan", "Bitmap Heap Scan"])
    assert a == b
...
def test_potpis_je_hashabilan_i_usporediv():
    ...
    assert plan_signature(["Seq Scan"]) == PlanSignature(
        uses_index=False, join_methods=frozenset()
    )
