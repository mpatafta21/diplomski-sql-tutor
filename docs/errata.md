...
| **#28** | DB `concepts.tier` ≠ Prolog tier za 6/30 koncepata | 🟡 **otvoren (zatečeno)** | BKT parametre određuje **Prolog** (`create_bkt_for_concept` → `engine.get_tier`) — on je autoritativan (CLAUDE.md). DB stupac `concepts.tier` divergira za: `insert` (DB medium / Prolog easy), `null_handling` (hard/medium), `cross_join` (medium/hard), `exists_subquery` (hard/medium), `in_subquery` (hard/medium), `scalar_subquery` (hard/medium). Praktična posljedica: **UI tier-chipovi** (iz `/modules`, dakle DB) mogu pokazivati drugu težinu nego što BKT stvarno koristi. Nije popravljano (backend/podaci zamrznuti); kandidat za Fazu 6 — uskladiti seed s ontologijom |
