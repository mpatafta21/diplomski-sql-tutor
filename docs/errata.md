...
| #17 | Frontend nema committed e2e suite | 🟡 otvoren | Brojke „N/N" u 4.1–4.3 bile su RUČNE verifikacije (headless Chrome/CDP), ne runner. Formulacije ispravljene u 4.4-0b. **Smoke suite = ulazni gate za eval (4.7)** 🔴 **PREBROJANO 2026-08-18.** Tvrdnja glasi da su brojke oblika „N/N” u 4.1–4.3 bile ručne verifikacije. Mjereno nad **git poviješću**, ne nad današnjim tekstom (formulacije su ispravljene u 4.4-0b, pa bi današnje brojanje podbrojilo): **prve verzije triju wrapupa nose ukupno 5 tvrdnji oblika N/N** (4.1 → 2, 4.2 → 0, 4.3 → 3); **danas je 1** (4.3), dakle **4 su ispravljena**, jedna je preživjela. **Što ih danas pokriva:** `frontend/e2e/` ima **4 testa** (smoke, submit-ishodi, hint-row, koncept-nav) + **1** kompletan prolaz u `e2e-prolaz/`. 🔴 **Pokrivenost nije 1:1 i ne smije se tako prikazati** — e2e testovi ne tvrde iste stvari nego da lanac prolazi; preostala „N/N” tvrdnja iz 4.3 **nema** automatiziranog parnjaka. Za rad: raspon je **5 → 1** ručnih tvrdnji uz **5 automatiziranih scenarija**, ne „sve je pokriveno”. |
| #18 | BKT `skill_mastery_history` je REKURZIVAN LANAC | ✅ procedura | Brisanje iz SREDINE lanca invalidira sve kasnije točke. Legitimno: (i) brisanje repa ili (ii) potpuni re-run. Kodirano u `scripts/purge_polluted_attempts.py` (default ODBIJA sredinu lanca) 🔴 **DUBINA IZMJERENA 2026-08-18** (podaci kompletnog prolaza, 88 zadataka jednog studenta): **29 koncepata, 339 točaka**; duljina lanca po konceptu **min 2 · medijan 6 · max 43**. Najdublji je `where_filter` (**43** točke), slijede `order_by` 40, `group_by` 33, `select_basic` 32. 🔴 **Brisanje jedne točke iz sredine najdubljeg lanca invalidira 22 kasnije točke** — jedan redak nosi **polovicu** povijesti tog koncepta. Procedura iz ovog nalaza (default ODBIJA brisanje iz sredine) time dobiva izmjerenu cijenu: nije rubni slučaj nego gubitak ~50 % krivulje po zahvatu. Razmjer: medijan je 6, pa je opasnost koncentrirana u nekoliko koncepata s mnogo **sekundarnih** ažuriranja (#31, #35). |
...
| **#39** | **`make dev-reset` guard je odbijao i ISPRAVNU potvrdu (`docker compose exec` jede stdin)** | ✅ **4.6-eval** | Prva verzija targeta ispisivala je stanje baze kroz `docker compose exec -T` **prije** `read -r ans`. `exec -T` i dalje pripaja stdin i **proždere ga**, pa je `read` dobivao prazan string i guard je odbijao brisanje čak i kad je korisnik upisao točno `OBRISI SVE`. Kvar je bio **zatvoren** (odbijao je brisati — sigurna strana), ali je target time bio neupotrebljiv. Popravak: `</dev/null` na svim naredbama koje prethode `read`-u. **Poučak:** guard koji nije testiran u OBA smjera nije guard — testirano je i da kriva potvrda prekida (exit ≠ 0, volumeni netaknuti) i da ispravna prolazi 🔴 **PREBROJANO 2026-08-18, opseg SUŽEN na `backend/agents/`** (puni backend uključio bi skripte i alate izvan studentskog puta — suženje se javlja, ne prešućuje). U `agents/` ima **16 `except` blokova**, od kojih **3 nose `# pragma: no cover`**: `coordinator.py:711` (`except ValueError`, „utrka s vlastitim `kill()`om”), `hint_llm.py:117` (`except ImportError`, SDK), `hint_agent.py:78` (`if task is None`, FK to sprječava). 🔴 **Nijedan nema test koji ih vidi AKTIVIRANE — 0 od 3.** To nije propust nego **deklaracija**: `# pragma: no cover` upravo znači „nijedan test ne izvršava ovu granu”; pretraga potvrđuje da nijedan test ne spominje `This behaviour is not registered` ni taj `ImportError`. 🔴 **Oštrija posljedica, spojena s #73:** guard na `coordinator.py:711` postoji za utrku koja se — izmjereno — **razrješava u drugu stranu**: naš `remove_behaviour` pobjeđuje (5/5), a iznimku baca SPADE-ov epilog **izvan** tog `try` bloka (20/20 predaja). Guard je dakle nedosežan i testom i u radu, dok stvarna iznimka prolazi pokraj njega. |
...
| **#80** | 🔴 **Savjet za `row_mismatch` promašuje jer mu `detail` NE IMENUJE razliku — „siguran” tip s nedoodređenim sadržajem** | 🔴 **otvoren; zaoštrava #64 i proširuje pravilo iz #72** | **Izmjereno na kompletnom prolazu kroz sučelje 2026-08-18** (88 zadataka, 130 predaja, 5 savjeta — `docs/e2e-kompletan-prolaz-wrapup.md` §C). ERRATA #72 je podijelila tipove na one kod kojih klasifikacija i payload ZAJEDNO određuju dijagnozu (→ LLM) i one kod kojih ne (→ katalog), i `row_mismatch` svrstala u **sigurne**, jer je u `DETAIL_SAFE_TYPES` i doista šalje `detail`. **Prolaz pokazuje da slanje detalja NIJE dovoljno — detalj mora IMENOVATI razliku.** `row_mismatch` `detail` dolazi iz `ComparisonResult.diff_summary` ([`sandbox_runner.py:295-368`](../backend/scripts/lib/sandbox_runner.py)) i glasi `Row 0 differs` ili `Row count mismatch: actual=7 vs expected=5` — kaže **da** se nešto razlikuje, nikad **što**. **Rezultat: 0/2 pogotka.** *(a)* Zadatak 35 (`having_filter`), student napisao `ORDER BY broj_proizvoda ASC` umjesto `DESC`, `detail` = `Row 0 differs` → savjet: *„Provjeri jesi li koristio HAVING klauzulu da filtriraš samo grupe gdje je brojač proizvoda veći od 6 — taj uvjet ne ide u WHERE, već nakon GROUP BY. Također provjeri koristi li tvoj upit GROUP BY category_id…”* — student je **oboje već napisao ispravno**, a **sortiranje se ne spominje**. *(b)* Zadatak 62 (`correlated_subquery`), student napisao `LIMIT 7` umjesto `LIMIT 5`, `detail` = `Row count mismatch: actual=7 vs expected=5` → savjet točno kaže *„vraća više redaka nego što bi trebalo”* (utemeljeno u detalju), pa nastavi nagađati kvar u koreliranome podupitu koji je student napisao **ispravno**. 🔴 **Kontrola u istom prolazu potvrđuje mehanizam:** tipovi čiji detalj IMENUJE (`wrong_columns` → `dobiveni: [… 'count' …], očekivani: [… 'distinct_products_sold' …]`; `plan_mismatch` → *„upit ne koristi indeks…; strategija spoja je Hash Join, a traži se Nested Loop”*) dali su **2/2 točna i korisna** savjeta koji imenuju pravi cilj; isto i `execution_error` na katalogu (**1/1**). **Dakle os nije „ima detalj / nema detalj” nego „detalj imenuje / detalj samo signalizira”.** **Za rad:** pravilo iz #72 ostaje ispravno ali NEPOTPUNO — particija po tipu ne hvata tip čiji je detalj prisutan a prazan po sadržaju. Kandidati (nijedan jednoredan, isto kao #64): obogatiti `diff_summary` imenovanom razlikom (dira ugovor usporedbe i `MONO_DETAIL_TYPES` u `feedback.ts`), premjestiti `row_mismatch` u katalog (gubi se ono malo točnosti koju ima), ili u promptu zabraniti tvrdnje o dijelovima upita koje detalj ne spominje. Srodno #64, #72, #79 |
| **#81** | 🔴 **`syntax_error` je kroz sučelje NEDOSTIŽAN — jedina grana koja ga proizvodi zatvorena je klijentskim gardom** | 🟡 **otvoren; taksonomija tvrdi granu koju student ne može doseći** | Izmjereno pri pripremi kompletnog prolaza (2026-08-18) i potvrđeno na prolazu: od 8 tipova u taksonomiji **7 je nastalo kroz sučelje, `syntax_error` nijednom** — i ne može. `evaluate()` ga vraća **isključivo** kad `sqlparse.parse(stripped)` da prazan rezultat ([`evaluation.py:320`](../backend/agents/evaluation.py)), a to je samo upit koji se strippa u prazno. Izmjereno nad kandidatima: `''`, `'   '`, `'\n\n'` → prazan parse; `';'`, `'--'`, `'-- komentar'`, `'/* komentar */'`, `'?'` → **svi daju 1 statement**, dakle prolaze dalje i padaju na `execution_error`. Ulaz je time sveden na prazan/whitespace upit — a njega sučelje **ne pušta**: `canSubmit = query.trim().length > 0` gasi gumb Submit, a `handleSubmit` isti gard ponavlja pa ga ni `Shift+Enter` ne zaobilazi ([`TaskPage.tsx`](../frontend/src/pages/TaskPage.tsx)). 🔴 **Gard je ispravan** (nula bespotrebnih zahtjeva) — nalaz nije da ga treba maknuti, nego da **taksonomija, katalog savjeta i `feedback.ts` nose granu koja u studentskom putu ne postoji**: `ERROR_TEXT.syntax_error` (*„Nisi poslao upit — editor je prazan…”*), `TEXT_DETAIL_TYPES` i `LLM_TYPES` sve je pokrivaju. Posljedica za rad: pri navođenju taksonomije treba reći da je `syntax_error` **dosežan samo izravnim pozivom `/attempt`**, inače brojka „8 tipova grešaka” implicira pokrivenost koje nema. Srodno #71 (kod koji podnosi stanje nije dokaz da stanje nastaje) |
| **#82** | 🟡 **Stanje „Svi koncepti savladani” ne nastaje ni sa 88/88 riješenih — `repeat_practice` uvijek vrati `task_id`** | 🟡 **otvoren; ekran postoji, put do njega ne** | Izmjereno na kompletnom prolazu 2026-08-18. `TaskEntryPage` ima završnu granu za `recommendationKind === done` (`no_recommendation` → *„Svi koncepti savladani”*, `exhausted` → *„Nema novih zadataka”*). **Nijedna se nije pojavila**, ni nakon što su **sva 88 aktivnih zadataka riješena**: preporučivač i tada vraća `task_id` uz `reason: repeat_practice`, pa `/task` uredno preusmjeri na (već riješen) zadatak. Izmjereno: zadnjih 6 preporuka u prolazu = `task 39, repeat_practice`. **Dvije posljedice.** *(1) Mrtav ekran:* završna kartica i njezin tekst (uključujući `no_recommendation` formulaciju koju je #54-krug pažljivo prepravljao) **ne mogu se vidjeti u normalnom radu** — postoje samo u jediničnim testovima. *(2) Put se zavrti u mjestu:* nakon 57. zadatka „Sljedeći zadatak” počne nuditi isti već riješen zadatak, a **31 zadatak je još neriješen** i dosežan samo klikom po konceptu u Modulima. Student koji se oslanja na CTA u tom trenutku **razumno zaključi da je gotov**. Kartica pošteno piše *„Ovaj si zadatak već riješio — ponovi ga za vježbu”*, ali nigdje ne kaže **da drugdje ima neriješenih**. Srodno #77 (isti korijen — doseg preporučivača), #54 |
| **#83** | 🟢 **Zalutali ćirilični znak u tekstu savjeta (`vidišь`)** | 🟢 **kozmetika; zabilježeno jer je u tekstu koji student čita** | Izmjereno 2026-08-18, savjet #3 kompletnog prolaza (zadatak 3784, `plan_mismatch`, `source='llm'`): *„Provjeri EXPLAIN plan za svoj upit: **vidišь** li Hash Join ili Nested Loop?”* — ćirilični meki znak `ь` (U+044C) usred hrvatske riječi. Savjet je inače **točan i koristan** (imenuje `CAST` anti-obrazac te Hash Join/Nested Loop), pa je ovo isključivo pitanje dojma. Nije `hintSegments` artefakt (#65) — znak je došao iz modela. **Ne popravlja se**: prompt je u zamrznutom opsegu, a jedini pouzdan popravak bio bi filtriranje izlaza modela, što je nerazmjerno jednom znaku. Bilježi se da se pri citiranju savjeta u radu ne pomisli na tipfeler u transkripciji. |
| **#84** | 🟡 **Ispuštanje `ORDER BY` ocjenjuje se TOČNIM iako zadatak izrijekom traži sortiranje — `order_matters` se izvodi iz PREDANOG upita** | 📌 **ODLUKA 2026-08-18: NE POPRAVLJA SE** — jednoredni popravak izmjeren i odbačen; ispravno rješenje traži `order_required` po zadatku (v. dolje) | Izmjereno **uživo kroz sučelje** 2026-08-18 (zadatak 13, `order_by`, *„Dostupni proizvodi sortirani po ID-u”*). Student je najprije predao `ORDER BY id DESC` → `row_mismatch`, *„Djelomično”*, +20 XP. Zatim je **maknuo `ORDER BY` u cijelosti** — `SELECT id, name, price FROM products WHERE stock > 0 LIMIT 3` — i dobio **„Točno” + 30 XP**. **Mehanizam:** `compare()` određuje `order_matters` regexom nad **`query`, a `query` je studentov upit** ([`sandbox_runner.py:306-308`](../backend/scripts/lib/sandbox_runner.py)); nema li u njemu `ORDER BY`, usporedba prelazi na **skupovnu** i redci se poklope. Isto vrijedi za zadatak 14 (provjereno offline kroz `evaluate()`). 🔴 **Nije kvar `compare()`:** za zadatke koji sortiranje ne traže izvođenje iz predanog upita je razumno i namjerno. Kvar je što **ništa ne veže tvrdnju zadatka („sortirano po X”) uz način ocjenjivanja** — pouka zadatka i kriterij ocjene su odvojeni, pa student koji preskoči ono što zadatak traži dobiva punu potvrdu i pun XP. Pogađa najmanje `order_by` (2 aktivna zadatka) i sve zadatke čiji opis traži poredak a referentni upit ima `ORDER BY` — dakle **veći dio banke**. **Kandidati** (nijedan jednoredan): `order_matters` izvoditi iz **`expected_query`** umjesto iz predanog (mijenja ugovor `compare()` i pogađa offline alate koji ga zovu), ili uvesti po-zadatku zastavicu `order_required` (migracija sheme + revizija 88 zadataka). Srodno #66 (isti razred: zadatak tvrdi nešto što ocjenjivanje ne provjerava) 🔴 **DOPUNA 2026-08-18 — OPSEG IZMJEREN NAD SVIH 88 AKTIVNIH, i veći je nego što je jedan zadatak sugerirao.** Metoda: svakom aktivnom zadatku čiji `expected_query` ima `ORDER BY` maknuta je ta klauzula (`LIMIT`/`OFFSET` zadržani, ista funkcija `m_bez_order_by` kojom je radio i prolaz) i puštena kroz `evaluate()`. READ-ONLY, bez perzistencije; brojke `COUNTED_TABLES` prije i poslije identične. **(1) `ORDER BY` ima 56/88 zadataka (63,6 %)**: M1 **15/15**, M6 **5/5**, M2 15/19, M0 5/7, M3 12/22, M5 4/10, M4 **0/10** (DML nema `ORDER BY`). **(2) Od tih 56, sustav ocijeni TOČNIM 44 — dakle STVARNI OPSEG JE 44/88 = 50,0 % svih aktivnih zadataka** (78,6 % onih s `ORDER BY`); preostalih 12 uhvati kao `row_mismatch`. **(3) Hipoteza „`LIMIT` štiti" je POLOVIČNO točna i zato opasna kao pravilo.** Izmjereno: **bez `LIMIT`-a otpornost je NEMOGUĆA — 19/19 takvih zadataka prođe kao točno**, i to nije slučajnost nego posljedica: bez `LIMIT`-a izostavljanje `ORDER BY` ne mijenja *skup* redaka, a usporedba je tada skupovna, pa se **mora** poklopiti. **Ali `LIMIT` nije dovoljan:** od 37 zadataka s `ORDER BY` **i** `LIMIT`-om, njih **25 svejedno prođe**, a samo 12 padne. Sva 12 otpornih ima `LIMIT` (0 bez njega). **(4) Što razlikuje tih 12:** prolaze oni kojima se traženi poredak poklapa s prirodnim izlaznim poretkom plana za prvih N redaka — tipično `ORDER BY <id>` nad svježe seedanom tablicom (zadaci 7–19, 47, 50, 56, 58, 62, 72, 78, 81, 3787…); padaju oni koji sortiraju po **izračunatoj** vrijednosti (`ORDER BY ukupna_potrosnja DESC`, `total_revenue DESC`, `prosjecna_ocjena DESC` — zadaci 28, 29, 31, 32, 33, 53, 54, 55) ili čiji izlaz slaže JOIN, a ne heap (39, 84, 6, 3788). 🔴 **Ta je „otpornost" dakle artefakt fizičkog poretka redaka u heapu, ne svojstvo ocjenjivanja** — isti razred hazarda koji je ERRATA #60 već izmjerila na preporučivaču. `VACUUM FULL`, reseed ili bilo koji `UPDATE` nad tablicom mogu bilo koji od tih 12 prebaciti u pogođene, pa je **44 donja granica, ne stabilna brojka**. **(5) Posljedica za rad:** tvrdnja „sustav provjerava sortiranje" ne stoji ni za polovicu banke; `order_by` kao koncept (2 zadatka, oba pogođena) **nema nijedan zadatak koji svoju pouku doista provjerava**. Kandidati za popravak nepromijenjeni; procjena troška sada je poznata — dira 56 zadataka, od toga 44 mijenjaju ocjenu. 🔴 **ODLUKA 2026-08-18 — POPRAVAK ODBAČEN, i to nakon što ga je mjerenje najprije obranilo.** Kandidat je bio jednoredan: `evaluation.py:410` proslijedi `expected_query` umjesto `submitted_query`, pa `order_matters` dolazi iz REFERENCE. **Obrana (sve izmjereno, READ-ONLY, `COUNTED_TABLES` prije i poslije identične):** *(a)* **izjednačenja nema nijednog** — 0/56 zadataka s `ORDER BY` ima dva retka s istim vrijednostima svih `ORDER BY` izraza (47 izmjereno izravno nad rezultatom, 8 po shemi jer je `id` jedini PK stupac u `ecommerce_v1` pa je UNIQUE NOT NULL, 1 ručno); *(b)* **referentni poredak je determinističan** — 0/56 upita promijenilo je poredak kroz 14 izvršavanja (8 čistih + 6 perturbacija planera: `enable_seqscan/indexscan/indexonlyscan/hashagg/sort/hashjoin/nestloop=off`); *(c)* **0 lažnih negativa** — simulirani popravak nad svih 88 referentnih rješenja ostavlja sva 88 `Točno`. Dakle rizik zbog kojeg se popravak dvojilo (uređena usporedba obara ispravna rješenja) **u ovoj banci ne postoji**. 🔴 **Obaranje:** isti simulirani popravak pušten je nad 56 rješenja BEZ `ORDER BY` i **uhvatio ih je samo 24 — 32 i dalje prolaze kao točna**. Popravak zatvara **12 od 44** pogođenih. Razlog: uređena usporedba prolazi kad student i bez `ORDER BY` dobije retke u istom poretku, a `ORDER BY <id>` nad svježe seedanom tablicom poklapa se s heap poretkom. **Presudno: zadaci 13 i 14 — OBA zadatka koncepta `order_by` — i dalje prolaze**, dakle popravak ne popravlja koncept zbog kojeg je uopće razmatran. Uz to bi **22 zadatka postala plan-ovisna** (toliko ih ima plan-ovisan poredak bez `ORDER BY`; danas to ne smeta jer je usporedba skupovna), od čega 3 (47, 50, 56) odmah s nestabilnim ishodom. **Trgovina je loša po svakom kriteriju: stabilna poznata rupa zamijenila bi se užom rupom s nestabilnim rubom.** 🔴 **PRAVA TVRDNJA KOJU MJERENJE NOSI:** rupu ne uzrokuje **odakle dolazi** `order_matters`, nego to što je poredak **izveden iz teksta upita umjesto deklariran u zadatku**. Popravak izvora ne dira uzrok. Ispravno rješenje je **`order_required` po zadatku** — migracija sheme + re-anotacija svih 88 zadataka; ograničenje s poznatom cijenom, što se u rad piše bolje nego popravak koji radi upola. **Opseg za citiranje:** pogođeno **najmanje 44 od 88** aktivnih (gornja granica 56; razlika ovisi o fizičkom poretku redaka i **nije stabilna**). `LIMIT` je **nužan ali nedovoljan** uvjet otpornosti — bez njega prolazi **19/19**, s njim svejedno prolazi 25 od 37. 🔴 **Mehanizam je već bio zapisan — kao ZAŠTITA.** ERRATA **#30** ga navodi kao ublažavanje drugog rizika: *„Rizik je dodatno ublažen time što `runner.compare` bez `ORDER BY` u upitu koristi **set-usporedbu** (redoslijed nebitan)"*. Isto ponašanje je ondje korist, ovdje rupa, i nitko ta dva retka nije spojio četiri faze. Obrnuti razred od #71 (ondje: kod koji podnosi stanje nije dokaz da stanje nastaje; ovdje: ponašanje zabilježeno kao pogodnost nikad nije ispitano kao hazard). **Četvrti primjerak granice rezultatske evaluacije, uz #29** (ne razlikuje ekvivalentne formulacije), **#30** i **#66** (ne može ocijeniti plan). |
| 🔒 **DOC** | **Tri pravila o instrumentu: nemoguć rezultat prijavljuje sebe · ponavljanje bez perturbacije ne mjeri determinizam · otpornost mjerena nad jednim fizičkim poretkom je donja granica** | 🔒 politika (iz #84, 2026-08-18) | **(1) Nemoguć rezultat je jedina besplatna dijagnostika.** Pri mjerenju #84 instrument je javio *„2 različita poretka"* za **svih 56** zadataka — uključujući one čiji rezultat ima **jedan redak**, gdje dva poretka ne postoje. Ta nemogućnost je otkrila kvar: `SandboxRunner.execute()` **ne prima planner flagove** (samo `explain()` ih ima), pa je `SET LOCAL enable_seqscan=off; <upit>` prošao kao multi-statement i psycopg je vratio rezultat **prve** naredbe — prazan. 🔴 **Pouka je asimetrična i zato vrijedi kao pravilo:** instrument koji proizvede nemoguć broj prijavljuje sam sebe, a instrument koji proizvede **uvjerljiv** krivi broj ne prijavljuje ništa i uđe u rad. Zato: prije nego se mjerenje javi, provjeri ima li u njemu vrijednost koja je **logički nemoguća** (poredak nad 1 retkom, postotak > 100, ista brojka za sve slučajeve). Zrcalna slika metodološkog zaključka projekta (#55: instrument za vrijednosti, ne za učinak). **(2) Ponavljanje bez perturbacije ne mjeri determinizam.** Osam identičnih izvršavanja daje isti poredak **i kad izjednačenja postoje**, jer je plan isti — potvrdilo bi determinizam koji ne postoji. Determinizam se mjeri **mijenjanjem plana**: `enable_seqscan/indexscan/indexonlyscan/hashagg/sort/hashjoin/nestloop=off`. Obrazac je posuđen iz `plan_is_stable` / `STABILITY_FLAGS` (#66) i **imenuje se kao posudba**, da se ne izvodi treći put iz početka. **(3) Otpornost izmjerena nad jednim fizičkim poretkom je DONJA GRANICA, ne brojka.** Od 56 zadataka s `ORDER BY`, 12 je „otporno" na izostavljanje — ali samo zato što im se traženi poredak **ne** poklapa s heap poretkom. `VACUUM FULL`, reseed ili bilo koji `UPDATE` mogu ih prebaciti među pogođene. Zato se u rad piše **„najmanje 44 od 88 (gornja granica 56)"**, nikad gola brojka. Isti razred hazarda kao #60 (preporuka ovisila o fizičkom poretku redaka u heapu). |
...
## <a id="biljeska-29"></a>#29 — opseg zaobilaska (mjereno 2026-08-18)

---

### 🔴 DOPUNA 2026-08-18 — OPSEG IZMJEREN: 20 od 30 koncepata, 61 od 88 zadataka

**Ispravak premise s kojom je mjerenje naručeno:** #29 nije bio „bez brojke". **ERRATA #84 je njegov prvi izmjereni primjerak** — izostavljanje `ORDER BY` daje `Točno` i pun BKT kredit za koncept `order_by`, na najmanje 44/88 zadataka. Pitanje zato nije je li #29 mjerljiv, nego **je li `order_by` iznimka ili obitelj**. Odgovor: obitelj.

**Metoda.** Za svakih 30 koncepata odgovoreno je postoji li formulacija koja daje **identične retke** a **ne koristi taj koncept**. Za svaki koncept razreda DA napisana je alternativa i dokazana **s tri neovisne provjere**, ne procjenom:

1. `runner.compare` → `matches = True`;
2. `evaluate()` → `is_correct = True` (isti put kojim ide studentov upit);
3. 🔴 `AstAnalyzer.detects_concept(alternativa, koncept)` → `detected = False`, **uz kontrolu** `detects_concept(referenca, koncept)` → `detected = True`.

Kontrola u točki 3 je nužna: bez nje „nije detektiran" ne znači ništa, jer detektor možda ne detektira ništa. READ-ONLY (`COUNTED_TABLES` prije i poslije identične, 199 attempta / 488 BKT točaka).

#### Razredi

| razred | koncepata | aktivnih zadataka | udio |
|---|---:|---:|---:|
| **DA — zaobilazak potvrđen** | **20** | **61** | **69,3 %** |
| NE — koncept je jedini put | 8 | 23 | 26,1 % |
| DJELOMIČNO | 1 (`null_handling`) | 4 | 4,5 % |
| bez primarnih zadataka | 1 (`join_condition`) | 0 | — |

**DA (20):** `group_by` 5 · `multi_table_join` 5 · `having_filter` 4 · `agg_count` 4 · `inner_join` 4 · `where_filter` 3 · `limit_offset` 3 · `agg_min_max` 3 · `left_join` 3 · `full_outer_join` 3 · `self_join` 3 · `in_subquery` 3 · `correlated_subquery` 3 · `column_alias` 3 · `distinct` 2 · `right_join` 2 · `cross_join` 2 · `scalar_subquery` 2 · `exists_subquery` 2 · `order_by` 2.

**NE (8):** `update` 4 · `delete` 4 · `from_clause` 3 · `agg_sum_avg` 3 · `index_usage` 3 · `select_basic` 2 · `insert` 2 · `explain_plan` 2. Obrazloženja: DML operacija **jest** koncept (nema zamjene koja mijenja iste retke uz isti `RETURNING`); `SELECT`/`FROM` su nužni za svaki rezultat; `agg_sum_avg` je **NE u praksi** jer sva tri aktivna zadatka traže `SUM`, a PostgreSQL nema drugu agregaciju zbrajanja (`AVG` je zamjenjiv sa `SUM/COUNT`, ali nijedan zadatak ne traži samo `AVG`).

#### 🔴 M6 je JEDINO mjesto gdje je mehanizam #29 zatvoren

`explain_plan` i `index_usage` su **razred NE iz drugog razloga**: anti-obrazac i dalje vraća **bajt-identične retke** (zadaci 3782, 3784 — izmjereno), ali `evaluate()` vraća **`plan_mismatch`**, ne `Točno`. Plan-presence evaluacija iz **#66** dakle rješava upravo ono što #29 opisuje — za dva koncepta. To je jedini postojeći protuprimjer u sustavu i pokazuje da rješenje nije nemoguće, nego **skupo po konceptu**.

#### Provjereno: 20 zadataka, prošlo 20/20

| koncept | zadatak | čime je koncept zamijenjen |
|---|---:|---|
| `where_filter` | 17 | filtar u `JOIN … ON` nad `VALUES` konstantom |
| `limit_offset` | 10 | `ROW_NUMBER()` prozorska funkcija |
| `distinct` | 5 | `GROUP BY` po svim stupcima |
| `group_by` | 30 | `DISTINCT` + korelirani agregat |
| `having_filter` | 35 | agregat omotan podupitom, filtar u vanjskom `WHERE` |
| `agg_count` | 20 | `SUM(1)` |
| `agg_min_max` | 24 | `ORDER BY … LIMIT 1` |
| `inner_join` | 44 | implicitni spoj zarezom + uvjet u `WHERE` |
| `left_join` | 48 | `NOT EXISTS` anti-join |
| `right_join` | 84 | `LEFT JOIN` sa zamijenjenim stranama *(doslovni primjer iz izvornog zapisa)* |
| `full_outer_join` | 41 | `UNION ALL` dvaju anti-joinova |
| `cross_join` | 39 | `LEFT JOIN … ON TRUE` |
| `self_join` | 57 | tablica se spominje **jednom** (`WHERE manager_id IS NOT NULL`) — FK jamči postojanje |
| `multi_table_join` | 51 | ugniježđeni `IN` podupiti; nijedan `FROM` nema više od jedne tablice |
| `scalar_subquery` | 77 | izvedena tablica u `FROM` |
| `in_subquery` | 74 | `JOIN` + `COUNT(DISTINCT)` |
| `exists_subquery` | 72 | `JOIN` + `DISTINCT` |
| `correlated_subquery` | 60 | `LEFT JOIN` anti-join, bez korelacije |
| `column_alias` | 3786 | alias **bez ključne riječi `AS`** (PostgreSQL to dopušta) |
| `order_by` | 13 | 🔴 potklas **#84** — `ORDER BY` izostavljen, usporedba postaje skupovna |

Sve tri provjere prošle su za **svih 20**: retci identični, `evaluate()` točan, AST detektira koncept u referenci a **ne** u alternativi.

#### Dva usputna nalaza iz istog mjerenja

🔴 **(a) Zadatak 1 (`null_handling`): AST detektor ne nalazi koncept ni u SAMOJ REFERENCI.** `SELECT COUNT(*) AS ukupno_narudzbi, COUNT(employee_id) AS narudzbe_sa_zaposlenikom FROM orders` ne sadrži nijednu oznaku rada s NULL-om (`COALESCE`, `IS NULL`, `NULLIF`) — pouka „`COUNT(stupca)` preskače NULL" je **implicitna**. Smjer je obrnut od #29: nije da student može zaobići koncept, nego da ga **referentno rješenje ne koristi eksplicitno**. Zato je `null_handling` razred DJELOMIČNO: na zadacima 1 i 4 (`COUNT`-temeljeni) koncept nije ni prisutan, dok na 2 i 3 (`COALESCE`) pokušana zamjena `CASE WHEN` **i dalje** biva detektirana (koristi `IS NULL`).

🔴 **(b) AST detektori za `explain_plan` i `index_usage` su PLACEHOLDERI** (`extra_info.placeholder = True`, *„runtime EXPLAIN ANALYZE deferred to Phase 6"*). Treća provjera za ta dva koncepta **nije izvediva** — javlja se, ne zaobilazi. Za njih zaključak počiva na točkama 1–2 i na #66.

#### Kako se citira u radu

> Od **30 koncepata, 20 dopušta formulaciju koja daje identične retke bez upotrebe koncepta**; oni su primarni koncept **61 od 88 aktivnih zadataka (69,3 %)**. Provjerena je po jedna alternativa za svaki od tih 20 koncepata; sve tri provjere prošle su u **20/20** slučajeva. Za **2 koncepta** (`explain_plan`, `index_usage`) zaobilazak postoji na razini redaka ali ga **plan-presence evaluacija hvata** (#66). BKT procjena po konceptu je time **gornja granica izloženosti, ne mjera upotrebe koncepta**.

🔴 **Što se NE tvrdi:** da su studenti u prolazu koristili zaobilazne formulacije — nisu, referentna rješenja su predana doslovno. Tvrdi se da je **zaobilazak dostupan**, i to na 69,3 % banke. Srodno #84 (izmjereni potklas), #35, #31, #66.
...
**🔴 MJERENJE IZVEDENO 2026-08-18 — I NE MOŽE ODLUČITI, ŠTO JE SAMO PO SEBI NALAZ.**

Nalaz sam propisuje mjerenje: *„izračunati udio identičnih uzastopnih `submitted_query` po
korisniku"*. Izvedeno nad **130 predaja** kompletnog prolaza kroz sučelje:

| mjera | vrijednost |
|---|---|
| predaja ukupno | 130 |
| uzastopnih parova | 129 |
| parova na **istom** zadatku | 41 |
| **identičnih uzastopnih upita** | **0** |
| zadataka s > 1 predajom | 34 |
| od toga sa svim upitima **različitima** | **34 / 34** |

🔴 **Ta nula NE opovrgava #58.** Prolaz je skriptiran tako da svaka predaja nosi **drukčiju
imenovanu grešku** — ponavljanje istog upita je **konstrukcijom isključeno**, pa mjerenje
ne može proizvesti pojavu koju traži. Rezultat je svojstvo **instrumenta**, ne sustava.

Isti razred kao **#57** („instrument kalibriran prema zatečenom stanju potvrđuje zatečeno
stanje") i kao pravilo iz 🔒 politike uz #84 („ponavljanje bez perturbacije ne mjeri
determinizam"): mjerenje čiji je ishod određen dizajnom mjerenja nije mjerenje.

**Za rad:** #58 ostaje **NEMJEREN**. Brojka 0/129 smije se navesti **samo uz ovu ogradu**,
ili se izostavlja. Jedini valjan izvor je promet **stvarnih** studenata, gdje ponovna
predaja nije unaprijed isključena — dakle podatak koji nastaje tek u evaluacijskoj sesiji.
