...
| Test-driven implementacija s 3 nezavisne code-review iteracije | 17 commit-ova, 132 nova testa, sve HIGH severity findings adresirane prije commit-anja taska |
...
### 3.4 Validacija ključnih dizajnerskih invarijanti

| Invarijanta | Vrijednost / status | Dokaz |
|---|---|---|
| AST analyzer false positive rate na quoted identifiers | 0% | `test_ast_analyzer.py` regression testovi iz code review #1 (`"where"`, `"select"` kao stupci ne triggeraju `where_filter` / `select_basic` detektore) |
| Validator early-exit (short-circuit) | Fail na razini 1 → razine 2/3 ne pokreću se | `test_task_validator.py` regression testovi (`syntax_invalid` blokira AST coverage check; `coverage_missing` blokira sandbox execution) |
| Prompt cache hit ratio nakon 1. poziva | 99.93% (2 873 cached / 2 875 total input) | Live test log iz `select_basic_d1_2fd7fa43.json` |
| UTF-8 round-trip za hrvatske dijakritike | Bez gubitka (input → API → JSON → file) | `test_generate_tasks_integration.py` testira "ž" iz "Narudžbe"; live test prošao s "dobavljača" u title-u |

### 3.5 Code review checkpointi i adresiranja
...
| Prompt template review prije 2B generation run-a | Anti-halucinacijska pravila (sample rows + `LIMIT N`) dodana tijekom 2A za `select_basic`. Treba provjeriti primjenjivost na ostale concept tipove (agregacije, JOIN-ovi, subqueries mogu trebati drugačije anti-halucinacijske heuristike). | pre-Faza 2B (prije generation run-a) | High |
...
- [ ] Napisati 27 preostalih concept YAML config-ova (procijenjeno 2-3 sata, šablonski rad)
  - **Optimizacija:** 27 YAML-ova se mogu meta-generirati kroz Claude API koristeći postojeća 3 sample-a (`select_basic`, `inner_join`, `left_join`) kao few-shot. To smanjuje rad s 2-3h ručnog pisanja na ~30 min review-a generated YAML-ova.
